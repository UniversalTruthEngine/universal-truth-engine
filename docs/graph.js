...
function addStars() {
  ...
  for (let i = 0; i < 1600; i++) {
    v.push((Math.random() - 0.5) * 2200, (Math.random() - 0.5) * 2200, (Math.random() - 0.5) * 2200);
  }
...
function setupUI() {
  ...
  fly.addEventListener("change", e => {
    if (e.target.value) flyToNode(e.target.value);
  });
...
function buildGraph() {
  ...
  graph.nodes.forEach(n => {
    ...
    const sphere = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({
        color,
        emissive: color,
        emissiveIntensity: 0.22 + ((n.dependency_centrality || 0) / max) * 0.45,
        roughness: 0.4,
        transparent: true,
        opacity: 1
      })
  ...
  graph.edges.forEach(e => {
    ...
    const line = new THREE.Line(
      ...
      new THREE.LineBasicMaterial({ color: 0x8492aa, transparent: true, opacity: 0.34 })
    );
    line.userData = e;
    scene.add(line);
...
async function loadHealth() {
  try {
    const response = await fetch("./data/topology-health-preview.json", { cache: "no-store" });
    const h = await response.json();
    document.getElementById("health").innerHTML =
      '<div class="meta"><p><strong>Map health:</strong> ' + h.node_count + " nodes, " + h.edge_count +
      " edges.</p><p>Missing dependencies: " + h.missing_dependencies.length + "</p></div>";
  } catch (e) {
    // Non-critical.
  }
}
...
function highlightChain(id) {
  ...
  nodeObjects.forEach((o, k) => {
    o.material.opacity = chain.has(k) ? 1 : 0.16;
    o.material.emissiveIntensity = chain.has(k) ? 0.65 : 0.04;
  });

  labelObjects.forEach((o, k) => {
    o.material.opacity = chain.has(k) ? 1 : 0.13;
  });
...
function clearHighlight() {
  ...
  edgeObjects.forEach(l => {
    l.material.opacity = 0.34;
    l.material.color.set(0x8492aa);
  });
}
...
async function showNode(n) {
  ...
  let proofCards = "";
  sections.forEach(s => {
    proofCards += '<div class="proof-card"><h4>' + escapeHtml(s.title) + "</h4><pre>" + escapeHtml(s.content) + "</pre></div>";
  });
  if (!proofCards) proofCards = "<p>Proof file unavailable.</p>";

  let deps = "";
  if ((n.dependencies || []).length) {
    deps = (n.dependencies || []).map(dep => '<span class="dep-link" data-dep="' + dep + '">' + dep + "</span>").join("");
  } else {
    deps = "<p>None</p>";
  }
  ...
  const html =
    ...
    '<div class="meta">' +
    "<p><strong>Domain:</strong> " + escapeHtml(n.domain) + "</p>" +
    "<p><strong>Confidence:</strong> " + escapeHtml(String(n.confidence_level)) + "</p>" +
    "<p><strong>Dependency centrality:</strong> " + escapeHtml(String(n.dependency_centrality)) + "</p>" +
    "</div>" +
    '<div class="tabs">' +
    '<button data-tab="summary">Summary</button>' +
    '<button data-tab="proof">Proof</button>' +
    '<button data-tab="dependencies">Dependencies</button>' +
    '<button data-tab="raw">Raw Proof</button>' +
    "</div>" +
  ...
  document.querySelectorAll(".dep-link").forEach(el => {
    el.addEventListener("click", () => flyToNode(el.dataset.dep));
  });
}
