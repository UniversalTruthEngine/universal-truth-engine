...
async function loadGraph(){
  ...
  document.getElementById("status").textContent=`Loaded ${graph.nodes.length} truths and ${graph.edges.length} dependency links.`;
}
...
function addStars(){
  ...
  for(let i=0;i<1600;i++){
    v.push((Math.random()-.5)*2200,(Math.random()-.5)*2200,(Math.random()-.5)*2200);
  }
  g.setAttribute("position",new THREE.Float32BufferAttribute(v,3));
  scene.add(new THREE.Points(g,new THREE.PointsMaterial({color:0x73839f,size:1.15,transparent:true,opacity:.38})));
}
...
function buildGraph(){
  ...
  graph.nodes.forEach(n=>{
    ...
    const radius=9+Math.sqrt((n.dependency_centrality||0)+1)*5.5;
    const color=domainColors[n.domain] || 0x83b6ff;
    const sphere=new THREE.Mesh(
      new THREE.SphereGeometry(radius,32,32),
      new THREE.MeshStandardMaterial({color,emissive:color,emissiveIntensity:.22+((n.dependency_centrality||0)/max)*.45,roughness:.4,transparent:true,opacity:1})
    );
    ...
    label.position.set(p.x,p.y-radius-16,p.z);
    scene.add(label);
  ...
  graph.edges.forEach(e=>{
    ...
    const line=new THREE.Line(
      ...
      new THREE.LineBasicMaterial({color:0x8492aa,transparent:true,opacity:.34})
    );
    line.userData=e;
    scene.add(line);
...
function makeLabel(text){
  ...
  const sprite=new THREE.Sprite(new THREE.SpriteMaterial({map:new THREE.CanvasTexture(c),transparent:true,depthWrite:false,opacity:.9}));
...
async function loadHealth(){
  try{
    ...
    document.getElementById("health").innerHTML=`<div class="meta"><p><strong>Map health:</strong> ${h.node_count} nodes, ${h.edge_count} edges.</p><p>Missing dependencies: ${h.missing_dependencies.length}</p></div>`;
  }catch(e){}
}
...
function applyFilters(){
  ...
  const visible=new Set(graph.nodes.filter(n=>{
    const text=!q || n.id.toLowerCase().includes(q) || n.title.toLowerCase().includes(q);
    const dom=domain==="all" || n.domain===domain;
    return text && dom;
  }).map(n=>n.id));
...
function highlightChain(id){
  ...
  chain.add(id);
  nodeObjects.forEach((o,k)=>{o.material.opacity=chain.has(k)?1:.16;o.material.emissiveIntensity=chain.has(k)?.65:.04;});
  labelObjects.forEach((o,k)=>{o.material.opacity=chain.has(k)?1:.13;});
  edgeObjects.forEach(l=>{
    const active=chain.has(l.userData.source)&&chain.has(l.userData.target);
    l.material.opacity=active?.92:.07;
    l.material.color.set(active?0xeef3ff:0x8492aa);
  });
}

function clearHighlight(){
  nodeObjects.forEach(o=>{o.material.opacity=1;o.material.emissiveIntensity=.25;});
  labelObjects.forEach(o=>o.material.opacity=.9);
  edgeObjects.forEach(l=>{l.material.opacity=.34;l.material.color.set(0x8492aa);});
}
...
async function showNode(n){
  ...
  let proofText="";
  if(n.proof_file){
    proofText=await loadText(n.proof_file);
  }
  const proofPreview=proofText || "Proof file not available in browser path. Open repository proof file from the Fact Vault.";
  let html=`<h2>${n.id}</h2><h3>${n.title}</h3><p>${n.summary||""}</p>
  <div class="meta">
  <p><strong>Domain:</strong> ${n.domain}</p>
  <p><strong>Confidence:</strong> ${n.confidence_level}</p>
  <p><strong>Dependency centrality:</strong> ${n.dependency_centrality}</p>
  <p><strong>Proof path:</strong> ${n.proof_file||"Not listed"}</p>
  </div>
  <div class="proof-section"><h3>Dependencies</h3>`;
  if(deps.length){
    deps.forEach(dep=>{html+=`<span class="dep-link" data-dep="${dep}">${dep}</span>`;});
  }else{
    html+="<p>None</p>";
  }
  html+=`</div><div class="proof-section"><h3>Proof</h3><pre>${escapeHtml(proofPreview)}</pre></div>`;
